  ...
  <div>
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <div v-for="pkg in packages" :key="pkg.id"
        ...
        <div class="relative overflow-hidden rounded-t-lg cursor-pointer" @click="openLightbox(pkg)">
          <img :src="resolveImage(pkg.image)"
        ...
        <div class="mt-3 flex gap-2 px-2">
          ...
          <button @click.stop="deletePackage(pkg.id)"
        ...
        <div class="relative max-w-6xl w-full max-h-[90vh]">
          ...
          <div class="flex flex-col md:flex-row gap-6">
            <div class="md:w-2/3 h-full overflow-hidden rounded-xl">
              <img :src="resolveImage(lightboxPackage.image)"
                class="w-full h-full object-contain transition-transform duration-300" :style="{ transform: zoomLevel }"
                @click="toggleZoom" />
            </div>
            <!-- Package Details -->
      ...
      <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          ...
          <div class="flex justify-end gap-3">
            <button @click="showDeleteConfirm = false" class="px-4 py-2 border rounded-lg hover:bg-gray-100">
              Cancel
            </button>
...
export default {
  ...
  methods: {
    ...
    async confirmDelete() {
      ...
      this.$emit('refresh')
      this.showTemplateNotification('Package deleted successfully!', "error");
    ...
    showTemplateNotification(message, type = 'success') {
      this.notificationMessage = message;
      this.notificationType = type; // 'success' or 'error'
